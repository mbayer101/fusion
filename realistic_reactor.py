...
print("Running Simulation with Realistic Reactor Dimensions (2m Minor Radius)...")
...
# --- 2. DEFINE REALISTIC GEOMETRY (in cm) ---
r_plasma = 200.0                 # 2.0 meter minor radius plasma
r_wall = r_plasma + 2.0          # 2 cm Tungsten first wall (202 cm)
r_multiplier = r_wall + 5.0      # 5 cm Lead multiplier layer (207 cm)
r_blanket = r_multiplier + 45.0  # 45 cm PbLi breeding blanket (252 cm)
r_struct = r_blanket + 5.0       # 5 cm RAFM structural shell (257 cm)
r_outer = r_struct + 100.0       # Vacuum boundary offset (357 cm)
...
# --- 3. SETTINGS & SCALED NEUTRON SOURCE ---
settings = openmc.Settings()
settings.batches = 50
settings.inactive = 10
settings.particles = 50000  # Increased particle count for larger volume statistics
settings.run_mode = 'fixed source'

source = openmc.IndependentSource()
source.space = openmc.stats.CylindricalIndependent(
    r=openmc.stats.Uniform(0.0, 190.0),      # Distributed across the 2m plasma core
    phi=openmc.stats.Uniform(0.0, 6.28318),
    z=openmc.stats.Uniform(-300.0, 300.0)    # 6-meter high cylinder section
)
...
# --- 6. EXTRACT RESULTS & CALCULATE METRICS ---
with openmc.StatePoint(sp_filename) as sp:
    ...
    damage_energy_mean = df_dpa['mean'].values[0]

    Ed_Fe = 40.0
    h_source = 600.0  # matches z-height span (-300 to +300)
    vol_struct = math.pi * (r_struct**2 - r_blanket**2) * h_source
    atoms_per_cm3 = (7.8 / 55.85) * 6.022e23
    total_atoms = vol_struct * atoms_per_cm3
    
    dpa_per_sec = (damage_energy_mean * source_rate) / (2.0 * Ed_Fe * total_atoms)
    dpa_per_fpy = dpa_per_sec * (365.25 * 24 * 3600)

    print('\n==================================================')
    print('      REALISTIC REACTOR SCALE RESULTS (2M)        ')
    print('==================================================')
    print(f"Tritium Breeding Ratio (TBR): {tbr_mean:.4f} +/- {tbr_std:.4f}")
    print('--------------------------------------------------')
    print(f"Structural Wall Radiation Damage: {dpa_per_fpy:.2f} DPA / FPY")
    print('--------------------------------------------------')
